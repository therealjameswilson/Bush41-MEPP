...
function toCandidate(record) {
  ...
  return {
    ...
    sourceNote: `Source: George H.W. Bush Library, Bush Presidential Records, National Security Council, ${SERIES_TITLE}, ${record.title || `Catalog record ${record.naId}`}${localIdentifier ? `, ${localIdentifier}` : ""}${date ? `, ${date}` : ""}. Folder-level source candidate; document-level classification, distribution, drafting, and place/time data require review.`
  };
}
